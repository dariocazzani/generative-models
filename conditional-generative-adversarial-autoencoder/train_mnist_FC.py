...
def train(options):
    ...
    with tf.variable_scope('Encoder'):
        encoder_output = encoder(X, c)

    with tf.variable_scope('Decoder') as scope:
        decoder_prob, decoder_logits = decoder(encoder_output, c)
    ...
    with tf.variable_scope('Discriminator_X') as scope:
        D_X_real = discriminator_x(X, c)
        scope.reuse_variables()
        D_X_fake = discriminator_x(decoder_prob, c)
                ...
                for epoch in range(options.epochs):
                    for iteration in range(n_batches):
                        ...
                        for _ in range(n_critic):
                            ...
                            sess.run(train_op_Dis_X, feed_dict={X: batch_x, c: batch_y})
                        ...
                        for _ in range(n_critic):
                            ...
                            sess.run(train_op_Dis_z, feed_dict={X: batch_x, z: batch_z, c: batch_y})
                        ...
                        if iteration % 50 == 0:
                            summary, ld_X, ld_z, la, le, acc_X, acc_z = sess.run(
                                    ...
                                    feed_dict={X: batch_x, z: batch_z, c: batch_y})
