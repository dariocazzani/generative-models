...
def train(options):
    ...
    with tf.name_scope('Autoencoder'):
        with tf.variable_scope(tf.get_variable_scope()):
            ...
            decoder_output, logits = decoder(z_sample, c)
            generated_images = tf.reshape(decoder_output, [-1, 28, 28, 1])

    with tf.variable_scope(tf.get_variable_scope()):
        # Sampling from random z
        X_samples, _ = decoder(z, c, reuse=True)
    ...
    with tf.Session() as sess:
        ...
        if not options.run_inference:
            try:
                ...
                for epoch in range(options.epochs):
                    for iteration in range(n_batches):
                        ...
                        # Train
                        sess.run(train_op, feed_dict={X: batch_x, c: batch_y})

                        if iteration % 50 == 0:
                            summary, batch_loss = sess.run([summary_op, vae_loss], feed_dict={X: batch_x, c: batch_y})
            ...
            if len(experiments) > 0:
                ...
                plot(sess, z, X_samples, num_images=15, height=28, width=28, condition=True, c=c)
