...
from helpers.misc import check_tf_version, extend_options, generate_mask
...
def train(options):
    ...
    with tf.variable_scope('Discriminator') as scope:
        ...
        scope.reuse_variables()
        D_fake = discriminator(decoder_output)

    with tf.name_scope('Loss'):
        with tf.name_scope('Reconstruction_Loss'):
            loss_reconstr = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=logits, labels=X))
                ...
                for epoch in range(options.epochs):
                    for iteration in range(n_batches):
                        ...
                        for _ in range(n_critic):
                            ...
                            sess.run(train_op_Dis, feed_dict={X: batch_x, X_noisy: noisy_batch})
                        ...
                        sess.run(train_op_autoenc, feed_dict={X: batch_x, X_noisy: noisy_batch})

                        if iteration % 50 == 0:
                            summary, ld, la, acc = sess.run(
                                    [summary_op, loss_discriminator, loss_autoencoder, accuracy_tot],
                                    feed_dict={X: batch_x, X_noisy: noisy_batch})
